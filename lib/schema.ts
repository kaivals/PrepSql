...
async function runPostgresQuery(
  ...
  try {
    ...
    logQueryStep({
      type: "schema_discovery",
      sql:
        sql + (params.length > 0 ? ` [params: ${JSON.stringify(params)}]` : ""),
      success: true,
      executionTime: duration,
    });
    return res;
...
async function introspectSQLite(db: SqliteAdapter): Promise<SchemaTable[]> {
  ...
  for (const table of tables) {
    const columns = await runSQLiteAll(
      db,
      `PRAGMA table_info("${table.name}")`,
    );
    const fks = await runSQLiteAll(
      db,
    ...
    const indexesList = await runSQLiteAll(
      db,
    ...
    for (const idx of indexesList) {
      if (idx.unique === 1) {
        const idxCols = await runSQLiteAll(
          db,
    ...
    const sqlRow = await runSQLiteGet(
      db,
    ...
    const countRow = await runSQLiteGet(
      db,
...
async function introspectPostgres(pool: any): Promise<SchemaTable[]> {
  const tablesResult = await runPostgresQuery(
    pool,
    `
  ...
  for (const row of tablesResult.rows) {
    ...
    result.push({
      ...
      indexes: indexesResult.rows.map(
        (i: { indexname: string }) => i.indexname,
      ),
    });
  }
...
async function introspectMySQL(pool: any): Promise<SchemaTable[]> {
  ...
  try {
    ...
    const tableKey =
      Object.keys((tables as object[])[0] || {})[0] || "Tables_in_db";
    const result: SchemaTable[] = [];
    ...
    for (const row of tables as Record<string, string>[]) {
      ...
      const [countRows] = await runMySQLQuery(
        connection,
        `SELECT COUNT(*) as count FROM \`${tableName}\``,
      );
      const count = (countRows as { count: number }[])[0]?.count ?? 0;

      const [indexesRows] = await runMySQLQuery(
        connection,
        `SHOW INDEX FROM \`${tableName}\``,
      );
      const indexNames = Array.from(
        new Set((indexesRows as any[]).map((r) => r.Key_name)),
      );
