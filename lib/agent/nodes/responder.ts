...
export function responderNode(state: AgentStateType): Partial<AgentStateType> {
  ...
  if (state.intent === "out_of_scope") {
    const msg =
      "I'm focused on database queries. I can help you fetch, analyze, or modify data in your connected database.";
    const aiMessage = new AIMessage(msg);
    return {
      messages: [aiMessage],
      finalResponse: {
  ...
  if (state.finalResponse) {
    const msgText =
      state.finalResponse.message || state.finalResponse.explanation || "";
